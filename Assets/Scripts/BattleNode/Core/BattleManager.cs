...
public class BattleManager : MonoBehaviour
{
    ...
    public enum States { BattleStart, TurnStart, ActionSelection, UnitSelection, UnitSupport,UnitDefend, AreaSelection, UnitMove, Combat, TurnEnd, EnemyTurn, BattleEnd,  }    
    ...
    public void SetState(States nextState)
    {
        ...
        switch (nextState)
        {
            ...
            case States.TurnStart:
                foreach (Unit unit in UnitManager.Instance.allUnits)
                {
                    unit.BeforeTurnStart(); // 턴 시작 전 유닛 상태 업데이트
                }
            ...
            case States.TurnEnd:
                foreach (Unit unit in UnitManager.Instance.allUnits)
                {
                    unit.AfterTurnEnd(); // 턴 종료 후 유닛 상태 업데이트
                }
                foreach (Area area in AreaManager.Instance.allAreas)
                {
                    area.AfterTurnEnd(); // 지역 상태 업데이트
                }
                CheckVictoryCondition();
                turnCount++; // 턴 수 증가
            ...
            case States.EnemyTurn:
                ...
                StartCoroutine(EnemyAIUtility.ExecuteEnemyTurn());
                break;
            case States.BattleEnd:
                foreach (Unit unit in UnitManager.Instance.allUnits)
                {
                    unit.ToUnitData(); // 전투 종료 시 유닛 데이터를 저장
                }
                break;
        }
    }
